...
Written here by `djc-to-citry extension`. Install it on the engine that renders
these components:
...
Python, where the proxy shows: `x is True` and `x is None` are silently False,
and `re`, `str.join`, `os.fspath` and a validating `Kwargs` reject it. These
components were translated from django-components, which has no such marker, so
they are written as if there were none.
